...
\begin{align*}
  \vec{f}(\vec{v}):=\sum_{p\in\mathcal{P}(S)}^{}\prod_{i=1}^{n}\left((1-d_{v_i})^{1-\xi_p(i)}d_{v_i}^{\xi_p(i)}\right)\vec{g}(\vec{v}_p)
\end{align*}
with
\begin{align*}
  S:=\{x\in\mathbb{N} \mid 1\leq x\leq n\},\quad p\in\mathcal{P}(S)\\\\\\
  \text{indicator function:}\\\xi_A:\mathbb{N}\rightarrow\{0,1\},\quad A \subseteq \mathbb{N}\\
  \xi_A(x):=
  \begin{cases}
    1&x\in A\\
    0&x\not\in A
